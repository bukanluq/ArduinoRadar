...
class KeyboardMapperFrame(BaseSerialTool):
    ...
    def build_ui(self):
        ...
        self.log_textbox = ctk.CTkTextbox(self.hud_frame, font=("Consolas", 11), fg_color="#0A0F0D", text_color="#4CAF50", border_width=1, border_color="#1E1E1E")
        self.log_textbox.pack(side="right", fill="both", expand=True, padx=10, pady=10)
        self.log_textbox.configure(state="disabled")
        self.add_log("System Ready. Waiting for serial connection...")
    ...
    def refresh_mapping_list(self):
        ...
        for s_char, action in self.key_map.items():
            ...
            ctk.CTkLabel(row, text=s_char, font=("Segoe UI", 16, "bold"), text_color="#2ECC71", width=30).pack(side="left", padx=(15, 5), pady=10)
...
class RadarVisualizerFrame(BaseSerialTool):
    ...
    def build_ui(self):
        ...
        self.range_val_label = ctk.CTkLabel(self.conn_frame, text="100 cm", font=("Consolas", 12), width=50, anchor="w")
        self.range_val_label.pack(side="left")

        # 2. MIDDLE TIER: Canvas Frame
        ...
        if radius > 0:
            ...
            for blip in self.blips:
                if blip["size"] > 0:
                    ...
                    if color == "#FF3333": has_critical = True
            ...
            if closest_dist <= self.max_distance:
                ...
                self.hud_frame.configure(border_color=threat_color)
                self.canvas_frame.configure(border_color=threat_color)
            else:
                        ...
                        if len(parts) == 2 and parts[0].lstrip('-').isdigit() and parts[1].lstrip('-').isdigit():
                            ...
                            if 0 < distance <= self.max_distance:
                                ...
                                if radius > 0:
                                    ...
                                    threat_color = "#FF3333" if dist_pct < 0.33 else ("#FFD700" if dist_pct < 0.66 else "#00E5FF")
